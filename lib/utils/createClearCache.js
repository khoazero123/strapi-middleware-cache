 ...
 * @return {ClearCacheFn}
 */
function createClearCache(store, options, getCacheConfigByUid, logger) {
  /**
  ...
  return async function clearCache(cacheConf, params = {}) {
    ...
    const keysShouldDel = keys.filter(shouldDel);
    logger.debug(`[clearCache] keys: ${JSON.stringify(keys)}, regExps: ${regExps.join(',')}`);
    if (keysShouldDel.length) {
      logger.info(`Clear cache keys: ${keysShouldDel.join(',')}`);
      await Promise.all(keysShouldDel.map(del));
    }
  };
}
