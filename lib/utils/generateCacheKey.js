/**
 * @typedef {import('strapi-middleware-cache').ModelCacheConfig} ModelCacheConfig
 * @typedef {import('koa').Context} Context
 */

/**
 * Generates a cache key for the current request
 *
 * @param {ModelCacheConfig} cacheConf
 * @param {Context} ctx
 * @return {string}
 */
function generateCacheKey(cacheConf, ctx) {
  const querySuffix = Object.keys(ctx.query)
    .sort()
    .map((k) => `${k}=${JSON.stringify(ctx.query[k])}`) // query strings are key sensitive
    .join(',');
  const headersSuffix = cacheConf.headers
    .sort()
    .filter((k) => ctx.request.header[k.toLowerCase()] !== undefined)
    .map((k) => `${k.toLowerCase()}=${ctx.request.header[k.toLowerCase()]}`) // headers are key insensitive
    .join(',');

  let cacheKey = ctx.request.path;
  if (querySuffix || headersSuffix) {
    cacheKey += '?';
    if (!querySuffix) cacheKey += headersSuffix;
    else if(!headersSuffix) cacheKey += querySuffix;
    else cacheKey += `${querySuffix}&${headersSuffix}`;
  }
  return cacheKey;
}

module.exports = generateCacheKey;
